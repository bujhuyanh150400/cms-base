<div x-data='{
    ...
    x-init="$watch('openNav', val => localStorage.setItem('openNav', val))">
    <div class="relative h-full min-h-screen bg-[#0E162A] text-white duration-300" :class="openNav ? 'w-72' : 'w-20'">
        <div class="sticky top-5 left-0">
            <div class="relative flex flex-col text-white p-5">
                <button x-on:click="toggleNav"
                ...
                <div class="inline-flex items-center">
                    <div class="bg-amber-300 text-2xl text-blue-900 px-2 py-1 rounded-full block float-left mr-2 duration-500"
                        :class="!openNav && 'rotate-[360deg]'">
                        <i class="bi bi-dribbble "></i>
                    </div>
                    <h1 class="duration-200 font-bold" :class="!openNav && 'hidden'">{{ __('lang_admin.logo-admin') }}
                    </h1>
                </div>
                ...
                <ul class="flex flex-col gap-2">
                    @foreach ($list_menu as $key => $menu)
                        @if (isset($menu['space_menu']) && $menu['space_menu'] === true)
                            <li class="my-1" :class="!openNav && 'border-b'">
                                <span class="text-sm text-gray-400 font-bold"
                                    :class="!openNav && 'hidden'">{{ $menu['title'] }}</span>
                            </li>
                        @else
                            <li class="text-gray-200 text-sm flex items-center gap-2 cursor-pointer p-3 duration-200 hover:bg-[#1E283A] rounded-md @if (
                                !isset($menu['sub_menu']) &&
                                    $curret_route == $menu['route_name']
                            ) bg-[#1E283A] @endif"
                            ...
                            @if (isset($menu['sub_menu']))
                                <ul :class="subNav === {{ $key }} ? 'flex flex-col gap-2' : 'hidden'">
                                    @foreach ($menu['sub_menu'] as $sub_menu)
                                        <li @if ($curret_route == $sub_menu['route_name']) x-init="subNav = {{ $key }}" @endif
                                            x-data="{ isTooltipVisible: false }"
                                            class=" relative text-gray-200 text-sm flex items-center gap-2 cursor-pointer p-3 hover:bg-gray-500 rounded-md 
                                            @if ($curret_route == $sub_menu['route_name']) bg-gray-500 @endif"
